...
comp comp_s = [] (const PatchTreeKey& e1, const PatchTreeKey& e2, Dictionary& dict) {
    // If MSB is not set, id is HDT
    if (!(e1.get_subject() & 2147483648) && !(e2.get_subject() & 2147483648)) {
        return static_cast<int32_t>(e1.get_subject() - e2.get_subject());
    }
    //Else, translate to string and compare
    return e1.get_subject(dict).compare(e2.get_subject(dict));
};

comp comp_p = [] (const PatchTreeKey& e1, const PatchTreeKey& e2, Dictionary& dict) {
    // If MSB is not set, id is HDT
    if (!(e1.get_predicate() & 2147483648) && !(e2.get_predicate() & 2147483648)) {
        return static_cast<int>(e1.get_predicate() - e2.get_predicate());
    }

    //Else, translate to string and compare
    return e1.get_predicate(dict).compare(e2.get_predicate(dict));
};
comp comp_o = [] (const PatchTreeKey& e1, const PatchTreeKey& e2, Dictionary& dict) {
    // If MSB is not set, id is HDT
    if (!(e1.get_object() & 2147483648) && !(e2.get_object() & 2147483648)) {
        return static_cast<int>(e1.get_object() - e2.get_object());
    }
